...
struct MCMF
{
	...
	bool bellManFord(int s, int t, int &flow, int &cost)
	{
		...
		dist[s] = 0; inqueue[s] = 1; pre[s] = 0; a[s] = INF;
		...
		while (!q.empty())
		{
			...
			for (int i = 0; i < (int)(G[u].size()); i++)
			{
				...
				if (e.cap > e.flow && dist[e.to] > dist[u] + e.cost)
				{
					...
					{
						inqueue[e.to] = 1;
						if (dist[e.to] < dist[u]) q.push_front(e.to);
...
extern MCMF mincost;
//将数据流数组化
extern int GetData(int line_num, int num_user, int num_point, int num_edge, char*topo[], int **matrix_w,int*user_w, int*user);
